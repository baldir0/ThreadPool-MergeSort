...
// Class to hold merge sort functions
class Sort
{
public:
    template <typename T>
    static void merge(std::vector<T> &arr, unsigned int arrStart, unsigned int arrMiddle, unsigned int arrEnd, std::mutex &mtx)
    {
    ...
    template <typename T>
    static void mergeSort(std::vector<T> &arr, unsigned int arrStart, unsigned int arrEnd, ThreadPool &pool, std::mutex &mtx, short prevMergeId = -1, short parent = -1)
    {
        ...
        if (prevMergeId != -1 && pool.getTask(prevMergeId) != nullptr)
            pool.getTask(prevMergeId)->addRequires(MergeID);
        if (parent != -1 && pool.getTask(parent) != nullptr)
            pool.getTask(parent)->addRequires(MergeID);
        ...
        if (prevMergeId != -1 && pool.getTask(prevMergeId) != nullptr)
            pool.getTask(prevMergeId)->addRequires(firstSplitID);
        if (parent != -1 && pool.getTask(parent) != nullptr)
            pool.getTask(parent)->addRequires(firstSplitID);
        ...
        if (prevMergeId != -1 && pool.getTask(prevMergeId) != nullptr)
            pool.getTask(prevMergeId)->addRequires(secondSplitID);
        if (parent != -1 && pool.getTask(parent) != nullptr)
            pool.getTask(parent)->addRequires(secondSplitID);
